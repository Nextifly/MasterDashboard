...
const TableSity = ({ list: initialList, onHidden }: ITable) => {
  ...
  const handleSort = (columnName: string) => {
   if (sortedList) {
    ...
    newList.sort((a, b) => {
      ...
      if (columnName === '№' || columnName === 'Телефон' || columnName === 'Оформления заявок' || columnName === 'ID') {
          ...
          <tr className='h-15 text-center' key={rowIndex}>
            {row.map((cell, cellIndex) => (
              cellIndex !== 0 && (
                ...
                <td className={`font-medium text-[15px] ${cellIndex === 1 && 'text-start pl-2'}`} key={`${rowIndex}-${cellIndex}`}>
                  {cell}
                </td>
              ) : (
                <td className='font-medium text-[15px]' key={`actions-${rowIndex}`}>
                  <div className='flex justify-around items-center'>
                    <Image src={row[0] === 'false' ? ImageEye : SVGEye} alt='hidden' className='cursor-pointer size-[33px]' onClick={() => onHidden(cell, row[0])} />
                  </div>
                </td>
              )
              )
